...
def test_approve_is_idempotent() -> None:
    ...
    approved, wait_left, already_approved = store.approve_if_delay_passed(item.warning_id)
    assert approved is True
    assert wait_left == 0
    assert already_approved is False

    approved_again, wait_left_again, already_approved_again = store.approve_if_delay_passed(item.warning_id)
